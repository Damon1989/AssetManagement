...
using AssetManagement.Migrations.SeedData;
...
{
    public abstract class AssetManagementTestBase : AbpIntegratedTestBase<AssetManagementTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AssetManagementTestBase()
        {
        ...
        protected void UsingDbContext(Action<AssetManagementDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AssetManagementDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AssetManagementDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AssetManagementDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AssetManagementDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AssetManagementDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AssetManagementDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AssetManagementDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AssetManagementDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AssetManagementDbContext, Task<T>> func)
        {
            T result;
